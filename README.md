...
- `Dockerfile` — imagem multi-stage que compila o binário e produz uma imagem enxuta.
- `docker-compose.yml` — roda a instância do serviço com um container Postgres para desenvolvimento.
...
O `app` receberá `DATABASE_URL` apontando para `db` (serviço Postgres). Endpoints continuam em `http://localhost:8080`.

Buildar apenas a imagem do app:

```bash
docker build -t users-service:local .
```

Executar imagem (exemplo apontando para um Postgres remoto):
